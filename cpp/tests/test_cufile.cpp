...
TEST_CASE("Verify libcufile usage", "[test_cufile.cpp]")
{
    ...
    CUDA_ERROR(cudaMalloc(&unaligned_device, BLOCK_SECTOR_SIZE));
...
//     constexpr char const* test_flags_desc[] = { "regular file", "o_direct", "gds with no O_DIRECT", "gds with
//     O_DIRECT" }; constexpr int W_FLAG_LEN = sizeof(test_w_flags) / sizeof(test_w_flags[0]); constexpr char const*
//     test_r_flags[] = { "rpn", "rp", "rn", "r" };
//     // clang-format off
...
//         uint8_t* aligned_device_host = reinterpret_cast<uint8_t*>(ALIGN_UP(unaligned_device_host,
//         BLOCK_SECTOR_SIZE));
...
//         SECTION(fmt::format("Write Test with different parameters (offset:{}, count:{})", test_file_offset,
//         test_count))
//         {
//             {
//                 INFO("# unaligned_host");
...
//                         ssize_t write_cnt = fd->pwrite(unaligned_host, test_count, test_file_offset,
//                         test_buf_offset); if (flag_idx == 0)
//                         {
...
//                         size_t posix_hash = str_hash(std::string_view((char*)unaligned_host + test_buf_offset,
//                         read_cnt));
...
//                         size_t posix_hash = str_hash(std::string_view((char*)aligned_host + test_buf_offset,
//                         read_cnt));
...
//                             auto fd = cucim::filesystem::open(output_file.c_str(), test_w_flags[flag_idx]);
//                             cudaMemcpy(unaligned_device + test_buf_offset, test_data, test_count,
//                             cudaMemcpyHostToDevice); ssize_t write_cnt =
...
//                             ssize_t read_cnt = fd->pread(unaligned_device, test_count, test_file_offset,
//                             test_buf_offset); if (flag_idx == 0)
//                             {
//                                 reference_read_cnt = read_cnt;
//                             }
...
//                             auto fd = cucim::filesystem::open(output_file.c_str(), test_w_flags[flag_idx]);
//                             cudaMemcpy(aligned_device + test_buf_offset, test_data, test_count,
//                             cudaMemcpyHostToDevice); ssize_t write_cnt = fd->pwrite(aligned_device, test_count,
//                             test_file_offset, test_buf_offset); if (flag_idx == 0)
//                             {
//                                 reference_write_cnt = write_cnt;
//                             }
//                             else
...
//                             ssize_t read_cnt = fd->pread(aligned_device, test_count, test_file_offset,
//                             test_buf_offset); if (flag_idx == 0)
//                             {
//                                 reference_read_cnt = read_cnt;
//                             }
...
//                             auto fd = cucim::filesystem::open(output_file.c_str(), test_w_flags[flag_idx]);
//                             cudaMemcpy(
//                                 unaligned_device_host + test_buf_offset, test_data, test_count,
...
//                             auto fd = cucim::filesystem::open(output_file.c_str(), test_w_flags[flag_idx]);
//                             cudaMemcpy(
//                                 aligned_device_host + test_buf_offset, test_data, test_count,
//                                 cudaMemcpyHostToDevice);
//                             ssize_t write_cnt =
